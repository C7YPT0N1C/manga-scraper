...
def install_extension():
    ...
    try:
        ...
        if not os.path.exists(tarball_path):
            ...
            executor.await_async(_download)
...
def graphql_request(request: str, variables: dict = None, auth_user: str = None, auth_passwd: str = None, auth: bool = False, gql_debugging: bool = False):
    ...
    return executor.await_async(_aiohttp_request)
...
async def add_mangas_to_suwayomi(ids: list[int], category_id: int):
    ...
    # Call graphql_request (sync) ( use executor.await_async() ) from async context using executor.invoke()
    result = executor.await_async(
...
async def update_creator_manga(meta):
    ...
    sync contexts, callers should use executor.invoke(update_creator_manga, meta, type="TYPE").
...
def process_deferred_creators():
    ...
    This function remains synchronous. It calls async helpers via executor.invoke(..., type="TYPE") where necessary.
    # NOTE: type= 'gallery', 'image', or 'default' (generic task, if default, you don't need to specify type)
    """
    ...
    while process_creators_attempt <= max_attempts:
        ...
        if not nodes:
            ...
            for node in nodes:
                ...
                if os.path.exists(expected_path):
                    ...
                    # remove from deferred if found (async) -> use executor.await_async()
                    executor.await_async(remove_from_deferred, title)

            if new_ids:
                log(f"GraphQL: Adding {len(new_ids)} mangas to library and category.", "info")
                
                # add_mangas_to_suwayomi is async -> call via executor.await_async()
                executor.await_async(add_mangas_to_suwayomi, new_ids, CATEGORY_ID)
        ...
        # load_deferred_creators is async: call via executor.await_async()
        with _deferred_creators_lock:
            deferred_creators = executor.await_async(load_deferred_creators)
        ...
        for creator_name in sorted(deferred_creators):
            ...
            if manga_info.get("inLibrary") and CATEGORY_ID in category_ids:
                ...
                executor.invoke(remove_from_deferred, creator_name)
                continue
        ...
        if new_ids:
            # add_mangas_to_suwayomi is async -> call via executor.invoke()
            executor.invoke(add_mangas_to_suwayomi, list(new_ids), CATEGORY_ID)
    ...
    executor.invoke(save_deferred_creators, still_deferred)
...
def after_completed_gallery_download_hook(meta: dict, gallery_id):
    ...
    # Update creator's popular genres - update_creator_manga is async, call via executor.invoke()
    executor.invoke(update_creator_manga, meta)
