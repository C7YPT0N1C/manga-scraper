...
def _build_cached_metadata_entry(meta: dict, gallery_id: int) -> dict | None:
    if not meta or not isinstance(meta, dict):
        return None
    artists = APIGet.meta_tags("api", meta, "artist")
    groups = APIGet.meta_tags("api", meta, "group")
    languages = APIGet.meta_tags("api", meta, "language")
    return {
        "id": gallery_id,
        "title": meta.get("title", {}).get("english", f"Gallery {gallery_id}"),
        "artists": APIGet.artists(meta),
        "groups": APIGet.groups(meta),
        "tags": APIGet.tags(meta),
        "characters": APIGet.characters(meta),
        "parodies": APIGet.parodies(meta),
        "languages": APIGet.languages(meta),
        "pages": APIGet.page_count(meta),
    }
    #return {
    #    "id": gallery_id,
    #    "title": meta.get("title", {}).get("english", f"Gallery {gallery_id}"),
    ...
    #    "tags": APIGet.meta_tags("api", meta, "tag"),
    #    "characters": APIGet.meta_tags("api", meta, "character"),
    #    "parodies": APIGet.meta_tags("api", meta, "parody"),
...
def _load_master_cache() -> dict:
    ...
    try:
        ...
        scraperdb.prune_cache_metadata(cutoff)
        
        # Prune references to cache files by their own TTL
        scraperdb.prune_cache_references(time.time())
        
        metadata_block = scraperdb.load_cache_metadata_all(cutoff)
...
def _update_master_cache(entry_key: str, entry: dict):
    scraperdb.upsert_cache_reference(entry_key, entry)

def _remove_master_cache_entry(entry_key: str):
    scraperdb.delete_cache_reference(entry_key)
    
def prune_all_caches():
    """Centralised cache pruning for metadata, references, and files."""
    now = time.time()
    # Prune metadata entries in the master cache by their own TTL
    scraperdb.prune_cache_metadata(now - TTL)
    # Prune references to cache files by their own TTL
...
def sanitise_string(meta_or_title):
    ...
    possible_broken_symbols = scraperdb.load_broken_symbols()
    ...
    for i, page_info in enumerate(pages):
        ...
        if use_head_requests and page_info and meta.get("media_id"):
            try:
                ...
                if urls:
                    resp = APIGet.session(referrer="API", status="return").head(urls[0], timeout=(10, 10))
...
class LoadCache:
    ...
    def queued_galleries() -> list:
        ...
        return scraperdb.get_queued_galleries()
...
class APIGet:
    ...
    @staticmethod
    def artists(meta):
        return APIGet.meta_tags("api", meta, "artist") or ["Unknown Artist"]

    @staticmethod
    def groups(meta):
        return APIGet.meta_tags("api", meta, "group") or ["Unknown Group"]

    @staticmethod
    def tags(meta):
        return APIGet.meta_tags("api", meta, "tag") or []

    @staticmethod
    def characters(meta):
        return APIGet.meta_tags("api", meta, "character") or []

    @staticmethod
    def parodies(meta):
        return APIGet.meta_tags("api", meta, "parody") or []

    @staticmethod
    def languages(meta):
        return APIGet.meta_tags("api", meta, "language") or ["Unknown Language"]
...
class APIFetch:
    ...
    def gallery_ids(
        ...
        gallery_ids_session = APIGet.session(referrer="API", status="return")
                    ...
                    try:
                        ...
                        if attempt >= orchestrator.max_retries:
                            ...
                            if use_tor:
                                ...
                                time.sleep(wait)
                                gallery_ids_session = APIGet.session(referrer="API", status="rebuild")
                                try:
                                    resp = gallery_ids_session.get(url, timeout=(60, 60))
                                    resp.raise_for_status()
    ...
    def gallery_metadata(gallery_id: int):
        ...
        metadata_session = APIGet.session(referrer="API", status="return")
            ...
            try:
                ...
                if attempt >= orchestrator.max_retries:
                    ...
                    if use_tor:
                        ...
                        time.sleep(wait)
                        metadata_session = APIGet.session(referrer="API", status="rebuild")
                        try:
                            resp = metadata_session.get(url, timeout=(60, 60))
                            resp.raise_for_status()
                            return resp.json()
    ...
    def fetch_metadata_batch(gallery_ids: list) -> dict:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(APIFetch.gallery_metadata, gid): gid for gid in gallery_ids}
