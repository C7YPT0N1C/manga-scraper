...
def display_gallery_results(gallery_ids: list, cache_key: str = None) -> tuple[list, dict]:
    ...
    while True:
        ...
        summary = scraperapi.Get.metadata_summary(dict(metadata_items))
        logger.info(
    ...
    if select_choice in ("n", "none"):
        ...
        if input("\nApply filters to refine results? (y/n): ").strip().lower() == "y":
            ...
            summary = scraperapi.Get.metadata_summary(metadata)
...
def view_selected_galleries(selected_ids: list, cached_metadata: dict | None = None) -> list:
    ...
    selected_ids = scraperapi.Cache.Load.queued_galleries()
    if not selected_ids:
    ...
    cached_by_ids = scraperapi.Cache.Load.id_metadata(unique_ids)
    if cached_by_ids:
    ...
    if missing_ids:
        ...
        missing_metadata = scraperapi.Fetch.all_galleries_metadata(missing_ids)
    ...
    while True:
        ...
        summary = scraperapi.Get.metadata_summary(dict(active_items))
        logger.info(
            f"Summary: {summary['total_galleries']} galleries, "
            f"{summary['unique_artists']} artists, "
            f"{summary['unique_tags']} tags, "
...
def interactive_search_menu(initial_ids: list | None = None, unattended: bool = False):
    ...
    selected_ids = scraperapi.Cache.Load.queued_galleries() # Start with any galleries already in the queue (e.g. from previous searches or CLI flags)
    if initial_ids:
    ...
    def persist_selected_ids():
        ...
        cached_ids = scraperapi.Cache.Load.queued_galleries()
        merged = list(dict.fromkeys(cached_ids + selected_ids))
        scraperapi.Cache.Save.queued_galleries(sorted(set(merged)) if merged else [])
        selected_ids = merged
    ...
    while True:
        ...
        #------------------------------------------
            ...
            if ids and cache_key:
                cache_key = scraperapi.Cache.cache_keys("homepage", sort_val)
                if archive_mode:
        ...
        #------------------------------------------
            ...
            if search_query:
                ...
                if ids and cache_key:
                    cache_key = scraperapi.Cache.cache_keys("search", search_query)
                    if archive_mode:
                        selected_ids.extend(ids)
                        logger.info(f"Added {len(ids)} galleries to download list.")
                        logger.info(f"Total selected: {len(dict.fromkeys(selected_ids))} unique galleries")
        ...
        #------------------------------------------
            ...
            if query_value:
                ...
                if ids and cache_key:
                    cache_key = scraperapi.Cache.cache_keys(query_type, query_value)
                    if archive_mode:
                        selected_ids.extend(ids)
                        logger.info(f"Added {len(ids)} galleries to download list.")
                        logger.info(f"Total selected: {len(dict.fromkeys(selected_ids))} unique galleries")
