...
def download_gallery_multiartists(g,gallery_root):
    for artist in g["artists"]:
        ...
        metadata = {"title": g["title"], "author": artist, "artist": artist, "description": f"An archive of {artist}'s works.", "genre": g["tags"], "status": "0", "_status values": ["0=Unknown","1=Ongoing","2=Completed","3=Licensed"]}
        with open(os.path.join(doujin_folder,"metadata.json"),"w",encoding="utf-8") as f: json.dump(metadata,f,ensure_ascii=False,indent=2)
...
def retry_skipped(root):
    ...
    for line in open(temp_log,"r",encoding="utf-8").readlines():
        ...
        if gallery: download_gallery_multiartists(gallery,root); time.sleep(SLEEP_BETWEEN_GALLERIES)
...
def download_galleries_parallel(start_id,max_id,root):
    with ThreadPoolExecutor(max_workers=MAX_THREADS_GALLERIES) as ex:
        ...
        for fut in as_completed(futures):
            ...
            if g: download_gallery_multiartists(g,root)
            save_progress(gid); time.sleep(SLEEP_BETWEEN_GALLERIES)

def update_suwayomi():
    try: r=requests.post(SUWAYOMI_GRAPHQL,json={"query":"mutation { refreshLibrary }"},headers=SUWAYOMI_AUTH_HEADER or {},proxies=PROXIES if USE_TOR else None)
    except Exception as e: print(f"[!] Suwayomi error: {e}"); return
