...
def update_local_manifest_from_remote():
    ...
    for remote_ext in remote_manifest.get("extensions", []):
        if remote_ext["name"] not in local_names:
            ...
            logger.debug(f"\nAdded new extension to local manifest: {remote_ext['name']}")
...
def install_extension(extension_name: str):
    ...
    if not ext_entry:
        logger.error(f"\nExtension '{extension_name}' not found in remote manifest")
        return

    if ext_entry.get("installed", False):
        logger.info(f"\nExtension '{extension_name}' is already installed")
        return
    ...
    if hasattr(module, "install_extension"):
        ...
        logger.info(f"\nExtension '{extension_name}' installed successfully")
...
def uninstall_extension(extension_name: str):
    ...
    if hasattr(module, "uninstall_extension"):
        ...
        logger.info(f"\nExtension '{extension_name}' uninstalled successfully")
...
def get_selected_extension(name: str = "skeleton"):
    ...
    logger.error("\nSkeleton extension not found! This should never happen.")
    return None
