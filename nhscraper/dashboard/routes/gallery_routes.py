...
from nhscraper.core.orchestrator import DEDICATED_DOWNLOAD_PATH
...
def list_creators():
    creators = [
        name for name in os.listdir(DEDICATED_DOWNLOAD_PATH)
        if os.path.isdir(os.path.join(DEDICATED_DOWNLOAD_PATH, name))
    ]
    return jsonify({"creators": creators})

@gallery_bp.route("/list_galleries/<creator>", methods=["GET"])
def list_galleries(creator):
    creator_path = os.path.join(DEDICATED_DOWNLOAD_PATH, creator)
...
def view_image(creator, gallery, filename):
    ...
    gallery_path = os.path.join(DEDICATED_DOWNLOAD_PATH, creator, gallery)
