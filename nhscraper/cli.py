...
def _handle_gallery_args(arg_list: list | None, query_type: str) -> set[int]:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if m_homepage:
                    ...
                    result = executor.call_appropriately(fetch_gallery_ids, "homepage", None, sort_val, start_page, end_page)
                    if result:
                ...
                if m_query:
                    ...
                    result = executor.call_appropriately(fetch_gallery_ids, qtype, qvalue, sort_val, start_page, end_page)
                    if result:
                        gallery_ids.update(result)
                    ...
                    result = executor.call_appropriately(fetch_gallery_ids, "search", search_query, sort_val, start_page, end_page)
                    if result:
                        gallery_ids.update(result)
    ...
    for entry in arg_list:
        ...
        result = executor.call_appropriately(fetch_gallery_ids, query_lower, name, sort_val, start_page, end_page)
        if result:
            gallery_ids.update(result)
