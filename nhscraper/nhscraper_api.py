...
# UTILITIES
##################################################################################################################################
def fetch_galleries_by_creator(creator_type, name, page_start=1, page_end=None):
    # creator_type: 'artist' or 'group'
    # name: string, supports spaces
    # page_start: starting page to fetch (1-indexed)
    # page_end: ending page to fetch (inclusive). None = fetch all pages.
    import requests, urllib.parse

    query = f"{creator_type}:{name}"
    ...
    while True:
        ...
        url = f"https://nhentai.net/api/galleries/search?query={query_encoded}&page={page}"
        try:
            ...
            if not galleries:
                break
            ids.extend(g["id"] for g in galleries)
            if page >= data.get("num_pages", page):
                break
            page += 1
        except Exception as e:
            log_clarification()
            logger.warning(f"[!] Failed to fetch {creator_type} galleries page {page}: {e}")
            break
