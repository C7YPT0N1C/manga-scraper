...
from nhscraper.core.fetchers import *
...
def parse_gallery_list(arg=None, artist_arg=None, group_arg=None, tag_arg=None, parody_arg=None):
    ...
    if artist_arg:
        ...
        ids = fetch_galleries_by_artist("artist", name, page_start, page_end)
    ...
    if group_arg:
        ...
        digits = [int(p) for p in parts if p.isdigit()]
        page_start, page_end = (digits[0], digits[1]) if len(digits) == 2 else (1, None)
        ids = fetch_galleries_by_group("group", name, page_start, page_end)
        galleries.update(ids)
    
    # --tag parsing: "<NAME> [PAGE_START PAGE_END]"
    if tag_arg:
        parts = tag_arg.split()
        name = " ".join([p for p in parts if not p.isdigit()])
        digits = [int(p) for p in parts if p.isdigit()]
        page_start, page_end = (digits[0], digits[1]) if len(digits) == 2 else (1, None)
        ids = fetch_galleries_by_tag("artist", name, page_start, page_end)
        galleries.update(ids)

    # --parody parsing: "<NAME> [PAGE_START PAGE_END]"
    if parody_arg:
        parts = parody_arg.split()
        name = " ".join([p for p in parts if not p.isdigit()])
        digits = [int(p) for p in parts if p.isdigit()]
        page_start, page_end = (digits[0], digits[1]) if len(digits) == 2 else (1, None)
        ids = fetch_galleries_by_parody("group", name, page_start, page_end)
        galleries.update(ids)
